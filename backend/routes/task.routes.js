...
//no ver la informacion del vehiculo "eliminar"
...
//traer solo los datos que no estan "eliminados"
router.get("/activeData", async (req, res) => {
	try {
		const getOnlyHave = await Records.find({"status": 'true'});
		res.status(200).json(getOnlyHave);
	} catch (err) {
		res.status(500).json(err);
	}
...
router.get("/:id", async (req, res) => {
	try {
		const { id } = req.params;
		const getOnlyVehicle = await Records.findById({_id: id });
		res.status(200).json(getOnlyVehicle);
	} catch (err) {
		res.status(500).json(err);
	}
});
